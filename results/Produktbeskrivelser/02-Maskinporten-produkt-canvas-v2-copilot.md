...
## Kort beskrivelse
Maskinporten er Norges nasjonale felleskomponent for sikker autentisering og autorisasjon av maskin-til-maskin (M2M) kommunikasjon og API-kall mellom offentlige (og private) virksomheter. Den muliggjør at datakilder og datakonsumenter kan bygge sikre, skalérbare integrasjoner uten å måtte utvikle proprietære sikkerhetsløsninger. Maskinporten er ett av tre kjernelementer i Digdirs tilitsinfrastruktur sammen med ID-porten og eSignering.
...
- Gebyr for private API-konsumenter (ikke implementert per nå)
