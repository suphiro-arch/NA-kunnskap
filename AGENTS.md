...
- En ressurs skal bare kobles til kapabiliteter den selv er med på å realisere, ikke kapabiliteter den forbruker fra andre ressurser. Kapabilitetene er definert som evner: `Autentisering` er evnen til å verifisere en digital identitet, ikke det å ha innlogging. En fagløsning som logger inn med `ID-porten` eller `Feide`, forbruker den evnen, og skal derfor ikke ha `Tillit: Autentisering`. Koblingen hører hos `ID-porten` og `Feide`, som faktisk leverer den. Samme skille gjelder `Tilgangsstyring` og `Representasjon` for løsninger som bare får rettigheter delegert gjennom `Altinn Autorisasjon`.
- Testen er: ville kapabiliteten forsvunnet fra nasjonal arkitektur hvis denne ressursen ble borte? Er svaret nei, fordi en annen ressurs leverer evnen, skal koblingen ikke settes. At ressursen ikke ville fungert uten evnen, er ikke det samme som at den bidrar til den.
- Avhengigheten skal likevel beskrives, men som avhengighet. Den hører i `Gjenbruk`, `Relasjon til andre ressurser` eller tilsvarende felt, med navn på ressursen som leverer evnen. Det er informasjon som er verdt å ha; feilen er bare å føre den som kapabilitet.
- Grensetilfellet er ressurser som både forbruker og videreformidler en evne. En fellesløsning som tilbyr andre virksomheter tilgangsstyring som en del av sin egen tjeneste, realiserer kapabiliteten selv om den bygger på en annen ressurs. Er det tvilsomt, skal vurderingen begrunnes i forklaringen under kapabilitetspunktet, og valget logges i `briefs/decisions.md`.
...
- En ressursbeskrivelse skal stå alene som beskrivelse av ressursen. Den skal ikke begrunne hvorfor ressursen er interessant for nasjonal arkitektur, for arkitekturarbeid, for analysearbeid eller for ressursoversikten, og ikke omtale hva ressursen dekker eller mangler i forhold til oversikten. Unngå formuleringer som `I nasjonal arkitektur er ...`, `For arkitekturarbeid er ... relevant fordi`, `Ressursen er interessant i nasjonal arkitektur fordi`, `viktig i analysearbeid` og `i NA-sammenheng`. Skriv i stedet faktasetningen om ressursen selv: hvilken rolle den har i sitt eget domene, hva den henger sammen med, og hva som skjer hvis den ikke brukes.
- Skillet mot malseksjonene er at `Bruksområde`, `Typiske analyse- og beslutningssituasjoner` og tilsvarende felt skal si når ressursen bør brukes og hvilke beslutninger den støtter. Det er en egenskap ved ressursen. Å forklare hvilken verdi ressursen har for arbeidet med selve oversikten, er det ikke.
...
- Klassifiseringsbeslutninger som ikke er egenskaper ved ressursen selv, som valg av eierkode eller rammeverkskategori når saken har vært tvilsom, logges i `briefs/decisions.md`, ikke som brødtekst i ressursfila. Hva batchen inneholdt hører hjemme i `briefs/next-step.md`, men selve klassifiseringsvalget er en varig beslutning.
...
- Hvis en assistentkjøring endrer ressursbeskrivelser, `briefs/`, webgrunnlag eller andre tekstfiler som kan mates videre inn i ressursoversikten eller publiseringsflyten, skal mojibake rettes i samme kjøring før handover gis.
- Hvis en assistentkjøring oppretter nye ressursfiler eller andre filer som register, mapping eller publiseringsgrunnlag peker til, skal filene legges til i Git i samme kjøring før handover gis.
