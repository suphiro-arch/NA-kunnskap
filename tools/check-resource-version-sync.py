...
def check_nested_product_references(latest: dict[int, dict]) -> list[str]:
    """Produktreferanser utenfor `products` skal peke på siste versjon.

    Etter at den avledede `capabilities`-seksjonen ble fjernet 2026-09-16
    finnes det ingen slike referanser, så kontrollen er i praksis tom. Den
    beholdes som vern: dukker det opp en ny nestet struktur med
    produktpekere, blir utdaterte stier fanget med én gang i stedet for å
    ligge og råtne slik de gjorde i den forrige seksjonen.
    """
    findings: list[str] = []
    data = json.loads(MAP_FILE.read_text(encoding="utf-8-sig"))
