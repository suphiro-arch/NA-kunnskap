...
- Når kommunen mottar post fra andre offentlige virksomheter som sender via SvarUt, og vil få den rett inn i sak- og arkivsystemet.
- Når mottaket skal skje maskinelt framfor at noen laster ned og importerer forsendelser manuelt.
- Når forsendelsene er sensitive og må dekrypteres med kommunens egen private nøkkel.
- Når virksomheten trenger å oppdage at forsendelser ikke er hentet innen frist, slik at feil i mottakskjeden ikke blir stående ubemerket.

### Når SvarInn normalt ikke er førstevalg
- Når behovet er å sende. Da er `Fiks SvarUt` (`KS-003`) tjenesten.
- Når avsenderen ikke bruker SvarUt. SvarInn tar imot fra SvarUt-løpet, ikke fra vilkårlige kanaler.
- Når mottaket skal skje i en manuell innboks uten integrasjon. Hovedverdien i SvarInn ligger i maskinell import.
- Når virksomheten er statlig og alt har eFormidling som mottaksløp.
