...
## Kort beskrivelse
Kjernejournal er den nasjonale fellesløsningen for å gjøre utvalgte og viktige helseopplysninger tilgjengelige på tvers av behandlingssteder. Ressursen er viktig fordi den gir helsepersonell en felles oppslagsflate for informasjon som ellers lett blir fragmentert mellom virksomheter, journalløsninger og behandlingsnivåer.
...
Grunnlag: Kapabilitetene er beholdt fra gjeldende mapping, men tekstgrunnlaget er utvidet for å gjøre sammenhengen mellom funksjon, avgrensning og samhandling tydeligere.
...
- Analyseforbedringer: lagt inn tydeligere avgrensning mot EPJ og andre helseressurser, mer konkrete brukssituasjoner og svakheter, samt bedre kombinasjonsvurdering mot `Helsenorge`, `HelseID` og `e-resept`.
- Tekstlige forbedringer: utvidet `Hovedfunksjoner`, `Gjenbruk`, `Forretningsverdi` og `Risiko` til et nivå som gjør filen bedre egnet som beslutningsstøtte i analyser.
