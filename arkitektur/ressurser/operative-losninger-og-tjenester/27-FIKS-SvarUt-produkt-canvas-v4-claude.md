...
- Når kommunen skal sende vedtak, brev eller andre dokumenter til innbyggere og ikke selv vil håndtere valget mellom digital post, Altinn og print.
- Når avsender skal nå både privatpersoner og virksomheter med samme forsendelse, og kanalrekkefølgen er ulik for de to mottakertypene.
- Når utsendingen skjer fra sak- og arkivsystem og skal skje maskinelt som del av saksflyten.
- Når virksomheten trenger å følge opp leveransestatus og hendelser per forsendelse, ikke bare sende og glemme.
- Når mottakeren er i Norsk Helsenett og forsendelsen må rutes dit.

### Når Fiks SvarUt normalt ikke er førstevalg
- Når behovet er mottak av inngående post. Det dekkes av `SvarInn` (`KS-007`).
- Når behovet er varsling, SMS eller sikker innsending fra innbygger. Da er `Fiks melding` (`KS-002`) den bredere kanalplattformen.
- Når avsender er en statlig virksomhet. Statlige aktører bruker normalt `eFormidling` (`DIGDIR-007`) og `Digital postkasse` (`DIGDIR-009`) som sitt tilsvarende løp.
- Når dokumentet ikke er ferdig produsert. SvarUt formidler, men produserer ikke innhold.
...
- `SvarInn` (`KS-007`) er mottakssiden og forutsetter SvarUt i en fullstendig forsendelseskjede mellom offentlige virksomheter.
- `Fiks-plattformen` (`KS-001`) for felles identitet, integrasjon og forvaltning.
- `Digital postkasse` (`DIGDIR-009`) og `Altinn Melding` er mottakerkanaler i den prioriterte kanalrekkefølgen.
- `Kontakt- og reservasjonsregisteret` (`DIGDIR-005`) for å avgjøre om mottakeren kan nås digitalt.
- Kommunale sak- og arkivsystemer som avsendersystem.
