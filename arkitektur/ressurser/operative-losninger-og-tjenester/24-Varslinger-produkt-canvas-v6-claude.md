...
### Analyseforbedringer
- `Tillit: Tilgangskontroll` er fjernet. Kapabiliteten er evnen til å verifisere og håndheve en brukers rettigheter i sanntid når noen forsøker å utføre en handling. Altinn Varsling håndhever ingens tilgang: den slår opp autorisasjonsdata for å avgjøre hvem som skal varsles. Forklaringen i `v5` sa dette rett ut, med ordet `bruker`.
- Ingen informasjon går tapt. `Datadrevet: Sammenstilling av data` dekker allerede at produktet kombinerer kontaktdata, reservasjonsstatus og autorisasjonsinformasjon for å velge riktig mottaker og kanal, og avhengigheten til `Altinn Autorisasjon` er beskrevet under `Plattform`.
- Dette er siste post i gjennomgangen av tillitskoblingene i Altinn-familien, se [decisions.md](../../../briefs/decisions.md) 2026-09-14. De øvrige koblingene ble beholdt fordi håndhevingspunkter realiserer `Tilgangskontroll`; `24` skiller seg ut ved at det ikke skjer noen håndheving.

### Tekstlige forbedringer
- Ingen øvrige endringer. Teksten fra `v5` er beholdt uendret, siden revisjonen gjelder kapabilitetskoblingen alene.
