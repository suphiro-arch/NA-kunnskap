...
**Kildekode:** Ikke offentlig dokumentert. Selve portalen er ikke publisert som åpen kildekode. Enkelte komponenter, blant annet designsystemet og skjemakomponenter, er publisert på [github.com/helsenorge](https://github.com/helsenorge), de fleste under MIT-lisens.
...
- `Sluttbrukertjenester: Tjenestekjeder` er fjernet. Kapabiliteten er definert som evnen til dynamisk å sette sammen, koordinere og automatisere flyten av informasjon og prosesser på tvers av uavhengige, integrerbare tjenester. Forklaringen i `v2` var at brukeren kan bevege seg mellom informasjon, innlogging og flere tjenester i samme produktflate. Det er navigasjon i en felles inngang, og dekkes allerede av `Sammenhengende tjenester`. Kildene beskriver ikke at Helsenorge koordinerer eller automatiserer informasjonsflyt mellom de underliggende tjenestene. Samme vurdering ble gjort for `21` Altinn.no og `18` Norge.no.
- `Lisens` er lagt til som eget felt ved siden av `Kildekode`, som malen krever, og `Kildekode` sier nå at komponenter er publisert på GitHub selv om portalen ikke er det.
