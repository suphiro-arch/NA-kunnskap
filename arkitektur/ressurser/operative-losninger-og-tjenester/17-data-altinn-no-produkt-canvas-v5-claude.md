...
### Analyseforbedringer
- `Tillit: Autentisering` er fjernet. Kapabiliteten er evnen til å tilby innloggingsmekanismer og verifisere en digital identitet. data.altinn.no krever et gyldig token, men utsteder det ikke; identiteten verifiseres i Maskinporten eller gjennom virksomhetssertifikat. Avhengigheten er beskrevet under `Vanlige kombinasjoner med andre produkter`.
- `Tillit: Samtykke` er fjernet. Kapabiliteten omfatter mekanismer for innhenting, lagring, validering, logging, oppslag og tilbaketrekking av samtykker. data.altinn.no legger et samtykke eller annet rettsgrunnlag til grunn for deling, men frembringer og forvalter det ikke. Forholdet er beskrevet under `Hovedfunksjoner` og `Produktmål`.
- `Tillit: Tilgangskontroll` er beholdt. Tjenestekonteksten er produktets egen regelmodell, og håndhevingen skjer ved hvert oppslag. Det tekniske omfanget til kapabiliteten dekker eksplisitt tokenvalidering og integrasjon med felles mekanismer for identitet, representasjon og tilgangsstyring.

### Tekstlige forbedringer
- Ingen øvrige endringer. Teksten fra `v4` er beholdt uendret, siden revisjonen gjelder kapabilitetskoblingene alene.
