...
- Når kommunen trenger å varsle innbyggere raskt om noe tidskritisk, som driftsavbrudd, frister eller endringer i et tjenestetilbud.
- Når et dokument eller en fil er for stor eller for sensitiv til å sendes som vanlig e-postvedlegg.
- Når innbygger eller virksomhet skal kunne sende inn noe sikkert til kommunen uten å logge inn i et fagsystem, gjennom eDialog.
- Når samme brev skal ut til mange mottakere samtidig, og kommunen vil unngå manuell utsending per mottaker.
- Når kommunen vil ta i bruk én kanal først og utvide senere, siden kanalene kan aktiveres hver for seg.

### Når Fiks melding normalt ikke er førstevalg
- Når behovet er ren dokumentformidling til rett mottakerkanal med automatisk kanalvalg. Da er `Fiks SvarUt` (`KS-003`) den spesialiserte tjenesten.
- Når behovet er å motta inngående post fra andre offentlige virksomheter. Det dekkes av `SvarInn` (`KS-007`).
- Når kommunikasjonen krever full dialoghistorikk og saksbehandling i samme flate. Fiks melding er en kanalplattform, ikke en dialog- eller saksbehandlingsløsning.
- Når mottakeren er en statlig virksomhet som forventer forsendelse gjennom eFormidling. Da bør `eFormidling` (`DIGDIR-007`) vurderes som kanal.
...
- `Fiks SvarUt` (`KS-003`) og `SvarInn` (`KS-007`) når kommunikasjonen er dokumentformidling framfor varsling og dialog.
- `Kontakt- og reservasjonsregisteret` (`DIGDIR-005`) for kontaktopplysninger og reservasjon før utsending.
- Kommunale sak- og arkivsystemer, som er de vanligste avsender- og mottakssystemene.
...
- Nettkilde: https://github.com/ks-no (kildekodestatus for Fiks-plattformen, kontrollert 2026-09-05)
