...
### Analyseforbedringer
- `Tillit: Autentisering` er fjernet. Kapabiliteten er evnen til å tilby innloggingsmekanismer og verifisere en digital identitet, og modellen beskriver den som grunnlaget tilgangsstyring, tilgangskontroll og representasjon bygger på. Dialogporten krever et gyldig token, men verifiserer ingen identitet selv. Avhengigheten til ID-porten og Maskinporten er beskrevet under `Plattform`.
- `Tillit: Tilgangskontroll` er beholdt. Det tekniske omfanget til kapabiliteten dekker eksplisitt tokenvalidering og integrasjon med felles mekanismer for identitet, representasjon og tilgangsstyring. At beslutningen om autorisert part treffes i Altinn Autorisasjon, er derfor ikke til hinder for koblingen: Dialogporten håndhever den i sanntid når den filtrerer hvilke dialoger den enkelte får se.
