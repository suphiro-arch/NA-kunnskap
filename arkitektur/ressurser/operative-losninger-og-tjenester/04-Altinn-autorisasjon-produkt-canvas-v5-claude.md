...
Samtykke er en egen tilgangsform i produktet, ved siden av delegering og rettigheter. Tjenesteeier oppretter en samtykkeressurs i ressursregisteret og bestemmer hva som kan deles og hvor lenge samtykket er gyldig. Datakonsumenten ber om samtykke, sluttbrukeren sendes til Altinn for å godkjenne eller avslå, og resultatet hentes som samtykketoken gjennom `Maskinporten`.
...
- **Tillit: Samtykke** gir mekanismer for å definere en samtykkeressurs, innhente sluttbrukerens godkjenning, sette gyldighetstid, avslå, og gjøre resultatet tilgjengelig som samtykketoken for datakonsumenten.
...
Samtykke er en tredje tilgangsform, ved siden av delegerte rettigheter og tilgangspakker. Tjenesteeier definerer en samtykkeressurs i ressursregisteret, med hvilke data som kan deles, på hvilke vilkår og hvor lenge samtykket varer. Datakonsumenten sender en samtykkeforespørsel, sluttbrukeren sendes til Altinn for å godkjenne eller avslå, og svaret hentes som et samtykketoken via `Maskinporten`. Dette skiller seg fra delegering ved at det er den registrerte selv som gir tillatelsen, ikke en rettighetshaver som viderefører en rettighet.

En fjerde sentral funksjon er selve tilgangsevalueringen gjennom Policy Decision Point. Dokumentasjonen beskriver PDP som komponenten som avgjør om brukere og systemer er autorisert til å utføre en operasjon på en ressurs, basert på policyer og kontekst. Samtidig viser dokumentasjonen at Altinn autorisasjon støtter maskinelle scenarier gjennom systembruker og Maskinporten, noe som gjør produktet relevant for både sluttbrukersystemer og rene system-til-system-integrasjoner.
...
| Samtykkeressurser, samtykkedialog og samtykketoken | Selve datadelingen som samtykket åpner for, som skjer i den enkelte tjenesten |
...
---
- Nettkilde: https://docs.altinn.studio/en/authorization/guides/system-vendor/consent/ (kontrollert 2026-09-15)
- Nettkilde: https://altinn.github.io/docs/utviklingsguider/samtykke/ (kontrollert 2026-09-15)

## Endringer fra forrige versjon

### Analyseforbedringer
- `Tillit: Samtykke` er lagt til. `v4` nevnte ikke samtykke med ett ord, selv om samtykke er dokumentert som en egen tilgangsform i Altinn Authorization, ved siden av delegering og tilgangspakker.
- `Hovedfunksjoner` har fått et eget avsnitt om samtykkeflyten, og skillet mot delegering er gjort eksplisitt: ved samtykke er det den registrerte selv som gir tillatelsen, ikke en rettighetshaver som viderefører en rettighet.
- `Kort beskrivelse` og `Scope og avgrensning` skiller nå samtykkeressursen og samtykketokenet fra selve datadelingen samtykket åpner for, som skjer i den enkelte tjenesten.

### Tekstlige forbedringer
- Samspillet med `Maskinporten` er beskrevet konkret for samtykketoken, i tillegg til den eksisterende omtalen av systembruker.
- Kildegrunnlaget er utvidet med Altinns dokumentasjon for samtykke i Altinn 3 og den eldre utviklingsguiden.
