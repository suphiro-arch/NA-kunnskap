...
- **Tillit: Tilgangsstyring** forvalter scopes, klientregistrering og API-delegering som nasjonal fellesmekanisme for API-sikring og tokenforvaltning, slik at virksomheter kan definere, delegere og trekke tilbake maskinell tilgang til egne API-er.
...
### Analyseforbedringer
- `Tillit: Tilgangsstyring` er lagt til som kapabilitet. Det tekniske omfanget til kapabiliteten navngir Maskinporten eksplisitt som eksempel på nasjonale fellesmekanismer for API-sikring og tokenforvaltning, og scopes, klientregistrering og API-delegering utgjør livssyklusen til en maskinell rettighet.
- Skillet mot `Tillit: Tilgangskontroll` er at tilgangsstyringen er forvaltningen av rettigheten, mens tilgangskontrollen er håndhevingen når et token brukes.

### Tekstlige forbedringer
- Ingen øvrige endringer. Teksten fra `v4` er beholdt uendret, siden revisjonen gjelder kapabilitetskoblingen alene.
