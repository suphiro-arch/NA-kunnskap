# Produkt-canvas: Nasjonalt utdanningsregister

## Navn
Nasjonalt utdanningsregister

## Ressurs ID
SIKT-006

## Status/Livsfase
**Utviklet og innført som nasjonal registerressurs** - etablert som autoritativ kilde for utdanninger og læresteder.

**Fakta:** Sikt beskriver Nasjonalt utdanningsregister som en felles autoritativ kilde for norsk utdanning. Registeret skal samle informasjon om utdanninger og læresteder og gi hver utdanning en unik identifikator.

## Modenhet
**Middels til høy modenhet** - ny nasjonal registerressurs med tydelig mål og innføring:
- etablert for å samle data som tidligere var spredt
- skal støtte både aktive og historiske utdanningstilbud
- er finansiert av Kunnskapsdepartementet
- brukerflater og første utprøving er beskrevet i nyere kilder

## Kort beskrivelse
Nasjonalt utdanningsregister er den nasjonale registerressursen for informasjon om akkrediterte utdanninger og læresteder i Norge. Produktet samler data som tidligere var fordelt mellom mange kilder og gir utdanninger og læresteder en unik digital identitet som kan brukes på tvers av systemer, prosesser og analyser.

## Kapabiliteter
- **Datakilder: Grunndata** er relevant fordi registeret skal være autoritativ kilde for utdanninger og læresteder.
- **Datautveksling og integrasjon: Dele data med andre** er en kjernefunksjon fordi opplysningene skal kunne brukes på tvers av opptak, finansiering, rapportering og analyse.
- **Informasjonsforvaltning: Datastyring** er sentral fordi produktet skal sikre kvalitet, entydighet og ansvarlig forvaltning av utdanningsdata over tid.

## Produktmål
- være en autoritativ kilde for norske utdanninger og læresteder
- gi hver utdanning en entydig identifikator
- gjøre utdanningsdata enklere å finne, koble og gjenbruke

## Brukerbehov
- forvaltningen trenger en komplett og kvalitetssikret oversikt over utdanninger
- Lånekassen, opptak og andre trenger én felles kilde for verifisering og prosessering
- analyse- og statistikkmiljøer trenger koblingsbare og stabile utdanningsdata

## Hvem er brukerne og brukersegmentene
| Brukersegment | Primære behov | Bruksområde | Kommentar |
|---|---|---|---|
| Utdanningsforvaltningen | Autoritativ oversikt | Forvaltning og styring | Hovedbrukere |
| Lånekassen og andre etater | Verifisering og automatisering | Godkjenning, støtte og kontroll | Viktige gjenbrukere |
| Utdanningsinstitusjoner | Felles referansegrunnlag | Rapportering og samordning | Sektordeltakere |
| Analyse- og statistikmiljøer | Koblingsbare data | Innsikt og kunnskapsgrunnlag | Viktig sekundærbruk |

## Hovedfunksjoner
Nasjonalt utdanningsregisters viktigste funksjon er å samle informasjon om utdanninger og læresteder i én autoritativ nasjonal kilde. Produktet skal erstatte et mer fragmentert bilde der opplysninger tidligere har vært fordelt på flere kilder og kontekster, og gir dermed et felles grunnlag som kan brukes på tvers av forvaltning, analyse og digitale tjenester.

En annen sentral funksjon er å gi utdanninger og læresteder en entydig identitet som kan brukes i andre systemer og prosesser. Det gjør registeret relevant som koblingsressurs, ikke bare som et sted å slå opp informasjon. Når samme utdanning eller samme lærested skal gjenkjennes på tvers av opptak, finansiering, rapportering og analyse, er denne identitetsfunksjonen en viktig del av produktets verdi.

Produktet har også en tydelig datastyringsrolle. Registeret skal ikke bare lagre opplysninger, men bidra til at de er kvalitetssikrede, koblingsbare og bevarte over tid, inkludert historiske utdanningstilbud. Det betyr at løsningen både er et register og et virkemiddel for bedre informasjonsforvaltning i utdanningssektoren.

Samtidig er Nasjonalt utdanningsregister ikke hele studieadministrasjonen eller alle brukerflater i sektoren. Produktet skal være en felles registerressurs og identitetskilde, mens institusjonenes egne systemer, portaler og saksprosesser fortsatt ligger utenfor. Denne avgrensningen er viktig for å plassere løsningen på riktig nivå i porteføljen.

### Typiske brukssituasjoner (generisk)
- Når flere virksomheter trenger én autoritativ kilde for utdanninger og læresteder i stedet for lokale oversikter.
- Når systemer og prosesser trenger entydige identifikatorer for å kunne koble samme utdanning eller lærested på tvers av kontekster.
- Når forvaltningen trenger bedre datakvalitet, sporbarhet og styring av nasjonale utdanningsdata.
- Når analyse, finansiering, rapportering eller opptak er avhengige av et felles referansegrunnlag.

### Når Nasjonalt utdanningsregister normalt ikke er førstevalg
- Når behovet er studieadministrasjon, søknadsbehandling eller annen operativ saksflyt.
- Når behovet er deling av individuelle resultater, vitnemål eller personrettet dokumentasjon.
- Når brukeren trenger en sluttbrukerportal med veiledning, innsending eller dialog.
- Når oppgaven gjelder løpende domeneprosess i lokale fagsystemer, ikke felles registergrunnlag.

### Scope og avgrensning
| Inngår | Inngår ikke |
|---|---|
| Register over utdanninger og læresteder | Hele studieadministrasjonen ved institusjonene |
| Unike identifikatorer og basisopplysninger | Alle utdanningssektorens øvrige portaler og tjenester |

## Veikart over kommende funksjonalitet
**Fakta fra kildene (kontrollert 2026-03-27):**
- tiltaket 2023-2025 er beskrevet som gjennomført
- første brukerflater og utprøving er omtalt i 2025

## Forretningsverdi/Verdiforslag
- reduserer dublering og svak datakvalitet
- gjør automatisering på tvers av utdanningssystemet enklere
- styrker analyse, statistikk og verifisering

## Utfordringer og risiko
| Risikokategori | Konkret risiko | Håndtering |
|---|---|---|
| Samordning | mange datakilder og eiere kan gi ujevn kvalitet | tydelig forvaltningsmodell |
| Innføring | overgang fra spredte kilder til én kilde kan være krevende | gradvis innføring og pilotering |

## Kanaler
- https://fs.sikt.no/tjenester/nasjonale-register/utdanningsregister/
- https://sikt.no/tiltak/nasjonalt-utdanningsregister
- https://sikt.no/aktuelt/nasjonalt-utdanningsregister-pa-gang

## Plattform
Nasjonalt utdanningsregister er et nasjonalt register- og datastyringsprodukt for utdanningsdata.

## Gjenbruk
**Høy gjenbruksverdi** som nasjonal kilde for utdanningsdata på tvers av mange prosesser.

**Vanlige kombinasjoner med andre produkter:**
- Nasjonalt utdanningsregister + Opptaksløsninger når opptaksprosesser trenger entydige referanser til utdanninger og læresteder.
- Nasjonalt utdanningsregister + analyse- og statistikkløsninger når utdanningsdata skal kobles og sammenstilles på tvers av kilder.
- Nasjonalt utdanningsregister + sektorspesifikke fagsystemer når lokale prosesser trenger et felles nasjonalt referansegrunnlag.

## Støtter arkitekturprinsipper
- **P4: Del og gjenbruk data**
- **P5: Del og gjenbruk løsninger**
- **P7: Sørg for tillit til oppgaveløsningen**

## Finansiering
**Fakta:** tiltaket er beskrevet som finansiert av Kunnskapsdepartementet.

## Forvaltning/eier
| Ansvarsområde | Organisasjon / vurdering | Grunnlag |
|---|---|---|
| Produktansvar | Sikt | produktsidene |
| Finansiering | Kunnskapsdepartementet | tiltaksiden |

## Lenke til dokumentasjon
- https://fs.sikt.no/tjenester/nasjonale-register/utdanningsregister/
- https://sikt.no/tiltak/nasjonalt-utdanningsregister
- https://sikt.no/aktuelt/nasjonalt-utdanningsregister-pa-gang

## Kildegrunnlag brukt i utfyllingen
- Lokal fil: `config/prompts/produkt-canvas.system.md`
- Lokal fil: `config/templates/produkt-canvas-template.md`
- Lokal fil: `arkitektur/kapabiliteter/capabilities.yaml`
- Lokal fil: `arkitektur/prinsipper/principles.md`
- Lokal fil: `arkitektur/produkter/produktnummerering.md`
- Lokal fil: `sources/links.md`
- Nettkilde: https://fs.sikt.no/tjenester/nasjonale-register/utdanningsregister/ (kontrollert 2026-03-27)
- Nettkilde: https://sikt.no/tiltak/nasjonalt-utdanningsregister (kontrollert 2026-03-27)
- Nettkilde: https://sikt.no/aktuelt/nasjonalt-utdanningsregister-pa-gang (kontrollert 2026-03-27)
